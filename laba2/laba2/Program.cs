...
class Program
{
    static async Task Main()
    {
        ...
        File.WriteAllText("vacancies_designer.json", json2); // Сохраняем ответ в файл
        Console.WriteLine("Ответ от API hh.ru по запросу 'designer' сохранен в файл vacancies_designer.json");
        ...
        File.WriteAllText("vacancies_mgtu_stankin.json", json); // Сохраняем ответ в файл
        Console.WriteLine("Ответ от API hh.ru по запросу 'МГТУ СТАНКИН' сохранен в файл vacancies_mgtu_stankin.json");
        
        Thread.Sleep(5000);
        stopwatch.Stop();
        Console.WriteLine($"Время выполнения: {stopwatch.ElapsedMilliseconds} мс");
        //Console.WriteLine("Общее время работы: " + DateTime.Now.TimeOfDay);
    }
}